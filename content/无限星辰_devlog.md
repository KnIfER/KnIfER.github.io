## 公告
1. 论坛：[群](https://www.imdodo.com/channel/157568/889287)、[PDAWIKI论坛](https://www.pdawiki.com/forum/forum.php?mod=viewthread&tid=48462&page=1&extra=#pid2911837)、[freemdict论坛](https://forum.freemdict.com/t/topic/18443)、酷安话题。
2. 创造不易，助我优化：  
...
## v8.7.3
修复：分组管理中可能导致分组丢失的BUG。  
新增：长按备份分组至/sdcard/PLOD/CONFIG/beifen。  
...
## v8.6
新增：点击恢复页面标记的文本选择。  
优化：优化菜单，点击翻译、浮动窗口可用工具栏按钮切换分组。  
优化：修复键盘。  

## v8.5
新增：“查看剪贴板”页面可标记笔记。  
优化：修复lucene全文搜索在双引号中使用通配符。（lucene通配符只能用在词末，且双引号内需要是英文）  
优化：修复词典级别“加载同名css”  
优化：优化黑暗模式，可使用系统web的黑暗模式，不是反色、更稳定。  
优化：优化笔记，下划线厚度1px，可用右上菜单隐藏或模糊化。  
优化：优化笔记列表，点击后，自动滚动至标记位置。  


## 更新主题：优化交互

## v8.4
- 新增文本竖排：（词典设置）可开启文本竖排，以古法阅古诗。
- 新增列表返回动画：从内容页面返回词条列表时，为当前列表行播放缩放动画。
- 长按链接：可以选择/复制链接文本、在本页打开、弹窗打开等。
- 点击翻译窗口：长按返回按钮、标题栏、收藏按钮、发音按钮触发更多功能。
- [资源加载：调整从文件加载资源的策略](https://forum.freemdict.com/t/topic/18443/88?u=knifer)。
- **新增查看剪贴板**：右上角新增菜单 “查看剪贴板”，而且标记笔记后，会记录到“笔记列表”数据库中。
- 新增历史记录全选、多行复制功能。
- 兼容ety2023 -- 星号*应该不参与词条排序
- **新增两个扩展apk**：[复制搜索、搜索+按钮](https://forum.freemdict.com/t/topic/20416)，安装后增强文本选择菜单的功能。

## v8.3
1. 新增繁简通搜：作用于联合搜索，但只能处理一一对应，不处理一对多，且繁简结果没合并（需滑动列表）。
2. 优化谷歌翻译界面（参考[视频演示](https://www.bilibili.com/video/BV1Rb411d7Sj/)，需联“网”使用）。
- 输入框：搜索词历史新增“总是半折叠”、“总是收起”选项。
- 底部工具栏：图标可变小。（浏览选项）
- 滑动翻页：更灵敏。
